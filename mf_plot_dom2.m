...
m_grid('box', 'off', 'xtick', [], 'ytick', [], 'color', axcolor, 'Fontweight', 'Bold', 'FontSize', 11, 'linewidth', 0.5, 'linestyle', 'none'); hold on; % draw grid without box
...
set(gca, 'Fontsize', 12, 'Fontweight', 'Bold');                                                                    % set axes properties
...
if flag_cb == 1
    ...
    mf_freezeColors;
elseif flag_cb == 2
    cbh=colorbar('location', 'SouthOutside', 'Fontsize', 12, 'Fontweight', 'Bold', 'TickLength', [0 0], 'color', axcolor); hold on;     
    y1=get(gca,'position');
    y=get(cbh,'Position');
    y(4)=0.03;
    set(cbh,'Position',y)
...
for k=1:length(M.ncst) 
  m_line(M.ncst{k}(:,1),M.ncst{k}(:,2), 'color', bordercolor); hold on;                                          % plot country borders
end
hold on;


% plot coasts
m_gshhs('cc','color', bordercolor);                                                                                         % add coastline
...
elseif flag_cb == 2
    m_text(0,-120,cbcaption,'ver','bottom','hor','center', 'Fontweight', 'Bold', 'Fontsize', 12, 'color', axcolor); hold off;
end
